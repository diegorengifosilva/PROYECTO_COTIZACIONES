...
export default function CargoModal({ open, onClose, onGuardar, cargoData }) {
  ...
  useEffect(() => {
    if (cargoData) {
      setFormData({
        // Protección para el 0 como string
        codigo: cargoData.codigo !== undefined && cargoData.codigo !== null 
              ? String(cargoData.codigo) 
              : "",
        nombre: cargoData.nombre ?? "",
        nom: cargoData.nom ?? "",
        activo: cargoData.activo ?? true,
      });
    } else {
      setFormData({
        codigo: "NUEVO", // O dejar vacío si prefieres que lo escriban
        nombre: "",
        nom: "",
        activo: true,
      });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">
                {cargoData ? "Editar Cargo" : "Nuevo Cargo Maestría"}
              </h3>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                Configuración del catálogo de cargos
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="space-y-4">
            ...
            <InputField
              ...
              readOnly={!!cargoData} // Solo lectura si estamos editando
              inline
              size="sm"
              className={`font-mono font-bold ${cargoData ? 'bg-transparent border-none text-slate-500' : 'text-slate-800'}`}
              placeholder="Ej: C001"
            />
            ...
            <InputField
              ...
              onChange={handleChange}
              placeholder="Nombre principal del cargo..."
              inline
              size="sm"
            ...
            <InputField
              ...
              value={formData.nom}
              onChange={handleChange}
              placeholder="Nombre secundario o alias..."
              inline
              size="sm"
              className="font-semibold text-slate-700 italic"
            />
